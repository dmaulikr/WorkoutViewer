...
@objc open class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    open override func prepareForInterfaceBuilder() {
        ...
        set([10, 2, 34, 11, 22, 11, 44, 9, 12, 4], withLabels: ["One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten"])
    }
    ...
    fileprivate func addReferenceLines(inViewport viewport: CGRect) {
        ...
        referenceLineView?.set(self.range)
    ...
    fileprivate func updateFrames() {
        ...
        referenceLineView?.set(viewportWidth, viewportHeight: viewportHeight)
    ...
    open func set(_ data: [Double], withLabels labels: [String]) {
    ...
    fileprivate func animate(_ point: GraphPoint, to position: CGPoint, withDelay delay: Double = 0) {
    ...
    fileprivate func enqueue(_ animation: GraphPointAnimation) {
    ...
    fileprivate func dequeue(_ animation: GraphPointAnimation) {
    ...
    fileprivate func calculateRange(forActivePointsInterval interval: CountableRange<Int>) -> (min: Double, max: Double) {
    ...
    fileprivate func calculateRange(forEntireDataset data: [Double]) -> (min: Double, max: Double) {
        let range = calculateRange(for: self.data)
        return clean(range)
    }
    
    fileprivate func calculateRange<T: Collection>(for data: T) -> (min: Double, max: Double) where T.Iterator.Element == Double {
    ...
    fileprivate func clean(_ range: (min: Double, max: Double)) -> (min: Double, max: Double){
    ...
    fileprivate func graphWidth(forNumberOfDataPoints numberOfPoints: Int) -> CGFloat {
        let width: CGFloat = (CGFloat(numberOfPoints - 1) * dataPointSpacing) + (leftmostPointPadding + rightmostPointPadding)
        return width
    }
    
    fileprivate func calculatePosition(atIndex index: Int, value: Double) -> CGPoint {
    ...
    fileprivate func addStraightLineSegment(_ startPoint: CGPoint, endPoint: CGPoint, inPath path: UIBezierPath) {
        path.addLine(to: endPoint)
    }
    
    fileprivate func addCurvedLineSegment(_ startPoint: CGPoint, endPoint: CGPoint, inPath path: UIBezierPath) {
    ...
    fileprivate func activePointsDidChange() {
      ...
      if(shouldShowLabels) {
        ...
        updateLabels(deactivatedLabelPoints, activatedPoints: activatedLabelPoints)
      }
  }
    ...
    fileprivate func updateLabels(_ deactivatedPoints: [Int], activatedPoints: [Int]) {
    ...
    fileprivate func startAnimations(withStaggerValue stagger: Double = 0) {
        ...
        for i in pointsToAnimate {
            ...
            animate(point, to: newPosition, withDelay: Double(index) * stagger)
            index += 1
        }
...
private class BarDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    fileprivate var shouldRoundCorners = false
...
private class ReferenceLineDrawingView : UIView {
    ...
    fileprivate func createLabel(at position: CGPoint, withText text: String) -> UILabel {
    ...
    fileprivate func createIntermediateReferenceLines(in rect: CGRect, numberOfIntermediateReferenceLines: Int, for path: UIBezierPath) {
    ...
    @discardableResult
    fileprivate func recursiveCreateIntermediateReferenceLines(in rect: CGRect, width: CGFloat, for path: UIBezierPath, remainingPartitions: Int) -> UIBezierPath {
    ...
    fileprivate func createReferenceLineFrom(from lineStart: CGPoint, to lineEnd: CGPoint, in path: UIBezierPath) {
        if(shouldAddLabelsToIntermediateReferenceLines) {
            ...
            addLine(lineStart, to: lineEnd, in: path)
        }
    }
    
    fileprivate func addLine(withTag tag: String, from: CGPoint, to: CGPoint, in path: UIBezierPath) {
        ...
        addLine(from, to: to, withGaps: gaps, in: path)
    }
    
    fileprivate func addLine(_ from: CGPoint, to: CGPoint, withGaps gaps: [(start: CGFloat, end: CGFloat)], in path: UIBezierPath) {
        
        // If there are no gaps, just add a single line.
        if(gaps.count <= 0) {
            addLine(from, to: to, in: path)
        }
            ...
            addLine(from, to: gapLeft, in: path)
            addLine(gapRight, to: to, in: path)
        }
            ...
            addLine(from, to: firstGapLeft, in: path)
            ...
            addLine(lastGapRight, to: to, in: path)
        }
    }
    
    fileprivate func addLine(_ from: CGPoint, to: CGPoint, in path: UIBezierPath) {
        path.move(to: from)
        path.addLine(to: to)
    }
    
    fileprivate func boundingSize(forText text: String) -> CGSize {
        return (text as NSString).size(attributes: [NSFontAttributeName:labelFont])
    }
    
    fileprivate func calculateYAxisValue(for point: CGPoint) -> Double {
    ...
    func set(_ range: (min: Double, max: Double)) {
        self.currentRange = range
        self.referenceLineLayer.path = createReferenceLinesPath().cgPath
    }
    
    func set(_ viewportWidth: CGFloat, viewportHeight: CGFloat) {
